...
**What this costs.** The rewrite is a multi-quarter investment, not a 2026 release. Solo founder ramping into Rust from a TypeScript/Electron baseline; the realistic range to v2.0 parity is a year of focused work, not the back-of-envelope "4-6 months for an experienced Rust engineer" that gets quoted casually. The non-technical primary audience experiences zero workflow changes; the WASM layer is invisible to them. For the v1 user base on the day of v2 release, capabilities need a conversion path with two parts:

1. **Recorded steps and metadata.** Pure data; the v2 host reads the v1 JSON shape and re-emits it under the v2 manifest schema. Automated, runs on first launch.
2. **Host-resident logic.** v1 stores per-site or per-capability logic (validators, extractors, repair scoring) in the host TypeScript codebase, not in capability bundles. That logic does not auto-port into WASM components. The v2 host ships default behaviors that cover the common cases v1 logic was written for; capabilities that depended on behaviors the defaults don't cover are flagged on first launch with `needs review` and a specific reason. Conversion never silently drops behavior; if it can't preserve something, it says so.
